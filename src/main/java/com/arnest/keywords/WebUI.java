...
package com.arnest.keywords;

import com.arnest.constants.FrameworkConstants;
import com.arnest.driver.DriverManager;
import com.arnest.enums.FailureHandling;
import com.arnest.helpers.SystemHelpers;
import com.arnest.report.AllureManager;
import com.arnest.report.ExtentReportManager;
import com.arnest.report.ExtentTestManager;
import com.arnest.utils.BrowserInfoUtils;
import com.arnest.utils.DateUtils;
...
//    @Step("Get Current URL")
    ...
    public static void waitForUrlContain(String expectedUrl, int timeOutInSecond) {
        int elapsedTime = 0;
        while (elapsedTime < timeOutInSecond) {
        ...
        throw new RuntimeException("Timeout url did not match " + expectedUrl);
    }
     ...
     * @return true/false
     */
    //("Verify Page sources Contains Text {0}")
     ...
     */
    //("Select Option Dynamic by Text {0}")
     ...
     */
    //("Verify Option Dynamic Exist by Text {0}")
     ...
     */
    //("Get total of Option Dynamic with list element {0}")
     ...
     */
    //("Verify Selected Option by Text {0}")
     ...
     * @return true if options given selected, else is false
     */
    //("Verify Selected Option by Value {0}")
     ...
     * @return true if options given selected, else is false
     */
    //("Verify Selected Option by Index {0}")
     ...
     */
    //("Switch to Frame by ID or Name: {0}")
     ...
     */
    //("Switch to Window or Tab by Position: {0}")
     ...
     */
    //("Switch to Window or Tab by Title: {0}")
     ...
     */
    //("Switch to Window or Tab by Url: {0}")
     ...
     * @return true/false
     */
    //("Verify total of Windows or Tab")
     ...
     * @return true/false
     */
    //("Verify Alert present with timeout {0}")
    ...
    //("Check element exist {0} with retry {1} and timeout {2} ms")
     ...
     * @return true if the element has the desired text, otherwise false.
     */
    //("Verify text {1} of element [Contains] {0}")
     ...
     * @return true/false
     */
    //("Verify element Clickable {0} with timeout {1} second")
     ...
     * @return true/false
     */
    //("Verify element present {0} with timeout {1} second")
     ...
     * @return true/false
     */
    //("Verify element NOT present {0} with timeout {1} second")
     ...
     * @return true/false
     */
    //("Verify element visible {0} with timeout {1} second")
     ...
     * @return true/false
     */
    //("Verify element NOT visible {0} with timeout {1} second")
     ...
     * @return true/false
     */
    //("Drag from element {0} to element {1}")
     ...
     * @return true/false
     */
    //("Drag HTML5 from element {0} to element {1}")
     ...
     * @return true/false
     */
    //("Drag from element {0} to X={1}, Y={2}")
     ...
     */
    //("Set text on text box and press key")
     ...
     * @param by an element of object type By
     */
    //("Clear text in text box with Ctrl A")
     ...
     * @param value the value to fill in the text box
     */
    //("Clear and Fill text on text box")
     ...
     * @param by an element of object type By
     */
    //("Click on the element {0} with timeout {1}s")
     ...
     * @param by an element of object type By
     */
    //("Click on the element by Javascript {0}")
     ...
     */
    //("Check data by EQUALS type after searching on the Table by Column.")
     ...
     * @param value  value to compare
     */
    //("Check data by CONTAINS type after searching on the Table by Column.")
     ...
     * @return true/false
     */
    //("Verify element {0} with attribute {1} has value is {2}")
     ...
     * @return true/false
     */
    //("Verify element {0} has attribute {1} with timeout {2} second")
    ...
    /**
     * Select radio button by path
     *
     * @By Represent a web element as the By object
     */
    public static void selectRadioButon(String xpath) {
        By radioButton = By.xpath(xpath);
        WebUI.clickElement(radioButton);
        sleep(2);
    }

    /**
     * Drop down with take string path it will have text (which should be selected) and will click on that element
     *
     * @param xpath Represent a web element as the By object
     */
    public static void dropDown(String xpath) {
        By timeZonePathElement = By.xpath(xpath);
        WebUI.clickElement(timeZonePathElement);
        waitForPageLoaded();
        sleep(1);
    }

    /**
     * Multiple rules dynamically accepting rules object
     * Will look through rules
     * Will take the key and value
     * Will set the xpath dynamically from key
     * Will click on add new button to open new input field (according to the length of the rules)
     * Will add that value to that particular xpath
     *
     * @List Represent a key value object
     */
    public static void fillRulesInput(List<Map<String, String>> rules) {
        for (int i = 0; i < rules.size(); i++) {
            Map<String, String> rule = rules.get(i);
            // here we get key and value of rule
            for (Map.Entry<String, String> entry : rule.entrySet()) {
                String placeholder = entry.getKey();
                String value = entry.getValue();
                By ruleInput = By.xpath("//input[@placeholder='" + placeholder + "']");
                if (!WebUI.isElementPresent(ruleInput, 1)) {
                    WebUI.clickElement(By.xpath("//label[text()=\"Add more rules\"]"));
                    sleep(1);
                }
                WebUI.waitForElementVisible(ruleInput, 1);
                WebUI.setText(ruleInput, value);
    ...
    /**
     * Image upload method responsible for uploading images
     *
     * @By Represent a web element as the By object
     * @String Represent image path
     */

//    public static void imageUpload(By path, String imagePath) {
//        waitForPageLoaded();
//        sleep(2);
//        WebElement fileInput = WebUI.waitForElementPresent(path, 10);
//        getJsExecutor().executeScript("arguments[0].style.display = 'block';", fileInput);
//
//        String fullPath = Paths.get(SystemHelpers.getCurrentDir(), "src/test/resources/testdataunify/", imagePath).toString();
//
//        WebUI.uploadFileWithSendKeys(path, fullPath);
...
//    }
    ...
    public static void imageUpload(By path, String imagePath) {
        ...
        // Resolve the image path (relative or absolute)
        String fullPath = resolveImagePath(imagePath);
        System.out.println("path" + fullPath);
        // Upload the file using the resolved path
        WebUI.uploadFileWithSendKeys(path, fullPath);
        WebUI.waitForPageLoaded();
        WebUI.sleep(1);
    }
    ...
    /**
     * Resolves the image path to handle both relative paths and resources inside the JAR.
     *
     * @param imagePath The path to the image file (can be relative or absolute).
     * @return The full path to the image file.
     */
//    private static String resolveImagePath(String imagePath) {
//        // Check if the imagePath is an absolute path
////	    File file = new File(imagePath);
////	    if (file.exists()) {
////	        return file.getAbsolutePath();
////	    }
////
////	    // If the imagePath is not an absolute path, try to resolve it as a relative path
////	    String relativePath = Paths.get(SystemHelpers.getCurrentDir(), "src/test/resources/testdataunify/", imagePath).toString();
////	    file = new File(relativePath);
////	    if (file.exists()) {
////	        return file.getAbsolutePath();
////	    }
//
//        // If the file is not found in the file system, try to load it as a resource from the classpath
//        InputStream inputStream = WebUI.class.getClassLoader().getResourceAsStream("resources/testdataunify/" + imagePath);
//        if (inputStream != null) {
//            try {
//                // Create a temporary file to store the resource
//                File tempFile = File.createTempFile("temp", ".jpg"); // Change the extension if needed
...
//            } catch (IOException e) {
//                throw new RuntimeException("Error creating temporary file for resource: " + imagePath, e);
//            } finally {
//                try {
...
//
    private static String resolveImagePath(String imagePath) {
        String basePath;
        ClassLoader classLoader = WebUI.class.getClassLoader();

        // Check external directory first
        File externalResources = new File("dataFile/testdataunify/");
        if (externalResources.exists()) {
            ...
            // Check inside JAR resources
            URL resourceUrl = classLoader.getResource("resources/testdataunify/");
            if (resourceUrl != null) {
                basePath = "resources/testdataunify/";
                System.out.println("Loaded from JAR resources: " + basePath);
            } else {
                // Default to IDE execution
                basePath = "testdataunify/";
        ...
        // If not found, try loading from the JAR classpath
        InputStream inputStream = classLoader.getResourceAsStream(basePath + imagePath);
        if (inputStream != null) {
            try {
                File tempFile = File.createTempFile("temp", ".jpg"); // Change extension if needed
